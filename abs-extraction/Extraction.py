...
ap.add_argument("-u", "--url", type=str, required=True, help="Url to image")
...
for possibleC in possibleChars:
    ...
    def matchingChars(possibleC, possibleChars):
        ...
        # if the char we attempting to find matches for is the exact same char as the char in the big
        # list we are currently checking
...
for listOfMatchingChars in listOfListsOfMatchingChars:
    ...
    for i in range(0, len(plates_list)):
        ...
        # print(img)
        cv2.imwrite(temp_folder + "rootOriginal.png", img)
        cv2.imwrite(temp_folder + 'detectedOriginal.png', imgCropped)
        output = Functions.GetTextFromImage(temp_folder + 'detectedOriginal.png')
        finalOutput = ' '.join(output).encode('utf-8').strip()
        print (finalOutput)
